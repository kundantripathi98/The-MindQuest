...
function reducer(state, action){
  switch(action.type){
    ...
    case "tick": {
      return {...state, secondsRemaining: state.secondsRemaining - 1, status: state.secondsRemaining === 0 ? "finished" : state.status, highscore: state.secondsRemaining === 0 ? state.points > state.highscore ? state.points : state.highscore : state.highscore}
    }
    default: {
...
const App = () => {
  ...
  return <div className="app">
    ...
    <Main>
      ...
      {status === "error" && <Error/>}
      {status === "ready" && <StartScreen numQuestions={numQuestions} onClick={handleClick}/>}
      {status === "active" && <Question currentQuestion={questions[index]} dispatcher={dispatcher} answer={answer} index={index} numQuestions={numQuestions} points={points} maxPossiblePoints={maxPossiblePoints}/>}
